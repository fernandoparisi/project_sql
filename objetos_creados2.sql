...
SELECT * FROM Vista_Materiales_Detalle;
...
SELECT * FROM Vista_Ordenes_Trabajo_Extendida;
...
SELECT * FROM Vista_Documentos_Compras_Material;
...
SELECT * FROM Vista_Reservas_Material_OT;
...
SELECT * FROM Vista_Movimientos_Material_Proveedor;

-- Vista 6
CREATE VIEW Vista_Resumen_Compras_por_Material AS
SELECT
    M.ID_MAT,
    M.TEX_MAT AS Nombre_Material, 
    SUM(DC.CANT_PED) AS Cantidad_Total_Comprada, 
    SUM(DC.CANT_PED * DC.PREC_NETO) AS Monto_Neto_Total_Comprado, 
    COUNT(DC.ID_DC) AS Numero_Documentos_Compra 
FROM
    DocumentosCompras DC   
JOIN
    Materiales M ON DC.ID_MAT = M.ID_MAT
...
SELECT * FROM Vista_Resumen_Compras_por_Material;    

-- Vista 7
CREATE VIEW Vista_Resumen_Ordenes_Trabajo_por_Equipo AS
SELECT
    E.ID_EQ,                 
    E.DEN AS Denominacion_Equipo, 
    COUNT(OT.ID_OT) AS Cantidad_Ordenes_Trabajo, 
    SUM(OT.COS_TOT) AS Suma_Costos_Totales, 
    AVG(OT.COS_TOT) AS Costo_Promedio_por_OT 
FROM
    OrdenesTrabajo OT
JOIN
    Equipos E ON OT.ID_EQ = E.ID_EQ
GROUP BY
    E.ID_EQ,
    E.DEN;

SELECT * FROM Vista_Resumen_Ordenes_Trabajo_por_Equipo; 

-- Vista 8
CREATE VIEW Vista_Antiguedad_Equipos AS
SELECT
    E.ID_EQ,   
    E.DEN AS Denominacion_Equipo,   
    FE.NOM_FEQ AS Familia_Equipo, 
    FAB.NOM_FAB AS Fabricante, 
    E.A_CON AS Anio_Construccion,    
    YEAR(CURDATE()) - E.A_CON AS Antiguedad_Anios,
    (SELECT COUNT(OT.ID_OT) FROM OrdenesTrabajo OT WHERE OT.ID_EQ = E.ID_EQ) AS Cantidad_Ordenes_Trabajo_Historicas
FROM
    Equipos E                     
LEFT JOIN 
    FamiliasEquipos FE ON E.ID_FEQ = FE.ID_FEQ 
LEFT JOIN
    Fabricantes FAB ON E.ID_FAB = FAB.ID_FAB;
    
SELECT * FROM Vista_Antiguedad_Equipos LIMIT 10;
...
DROP TRIGGER IF EXISTS auditoria_stock;

CREATE TRIGGER auditoria_stock
AFTER UPDATE ON Materiales
FOR EACH ROW
BEGIN
    IF OLD.STK_DISP <> NEW.STK_DISP THEN
        INSERT INTO AuditoriaStockMateriales (
            ID_MAT,
            Stock_Anterior,
            Stock_Nuevo,
            Fecha_Cambio,
...
DROP TABLE IF EXISTS AuditoriaStockMateriales;
CREATE TABLE AuditoriaStockMateriales (
    ID_ASM INT PRIMARY KEY AUTO_INCREMENT,
    ID_MAT INT NOT NULL,
    Stock_Anterior INT,
    Stock_Nuevo INT,
...
SELECT * FROM Materiales WHERE ID_MAT = 1; 

UPDATE Materiales
SET STK_DISP = STK_DISP + 10 -- Aumenta el stock en 10 unidades
WHERE ID_MAT = 1; 

SELECT * FROM AuditoriaStockMateriales ORDER BY ID_ASM DESC LIMIT 5;

-- Trigger 2
DELIMITER // 
CREATE TRIGGER validar_fechas_ot
BEFORE INSERT ON OrdenesTrabajo
FOR EACH ROW
BEGIN
    -- Validar que la fecha de inicio (FH_INI) no sea posterior a la fecha de fin (FH_FIN)
    IF NEW.FH_INI > NEW.FH_FIN THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: La fecha de inicio (FH_INI) no puede ser posterior a la fecha de fin (FH_FIN)';
    END IF; 
END // 
...
INSERT INTO OrdenesTrabajo (COS_TOT, FH_INI, FH_FIN, ID_COT, ID_FEQ, ID_EQ)
VALUES (1500.00, '2025-07-01', '2025-07-15', 1, 1, 1);

SELECT * FROM OrdenesTrabajo;

-- Insertar un valor inválido
INSERT INTO OrdenesTrabajo (COS_TOT, FH_INI, FH_FIN, ID_COT, ID_FEQ, ID_EQ)
VALUES (2500.00, '2025-08-01', '2025-07-25', 1, 1, 1);
