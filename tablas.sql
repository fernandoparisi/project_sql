...
CREATE TABLE GruposCompras (
    ID_GC INT PRIMARY KEY AUTO_INCREMENT,
    GRU_COM VARCHAR(50)
);
...
CREATE TABLE ClasesMovimiento (
    ID_CMV INT PRIMARY KEY AUTO_INCREMENT,
    NOM_CMV VARCHAR(50)
);
...
CREATE TABLE FamiliasEquipos (
    ID_FEQ INT PRIMARY KEY AUTO_INCREMENT,
    NOM_FEQ VARCHAR(50)
);
...
CREATE TABLE Monedas (
    ID_MON INT AUTO_INCREMENT PRIMARY KEY AUTO_INCREMENT,
    COD_MON CHAR(3) UNIQUE NOT NULL,     -- e.g. 'USD', 'ARS', 'EUR'
    TEX_MON VARCHAR(50)             -- e.g. 'Dólares estadounidenses'
);
...
CREATE TABLE Movimientos (
    ...
    ID_UM INT,
    ID_PROV INT,
    ID_CMV INT, -- ¡Aquí está la columna ID_CMV que faltaba!
    FOREIGN KEY (ID_MAT) REFERENCES Materiales(ID_MAT),
    FOREIGN KEY (ID_UM) REFERENCES UnidadesMedida(ID_UM),
    FOREIGN KEY (ID_PROV) REFERENCES Proveedores(ID_PROV),
    FOREIGN KEY (ID_CMV) REFERENCES ClasesMovimiento(ID_CMV)
